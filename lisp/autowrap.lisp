...
(defclass jit-state ()
  ((ptr :initform (lightningfn-ffi::fn-jit-new-state) :reader jit-state-ptr :documentation "Foreign pointer from fn-jit-new-state"))
...
(defun free-state-ptr (state-ptr)
  ;;(format t "free-state-ptr ptr:~A~%" state-ptr)
  (lightningfn-ffi::fn-jit-clear-state-raw state-ptr)
  (lightningfn-ffi::fn-jit-destroy-state-raw state-ptr))
(defmethod initialize-instance :after ((self jit-state) &key)
  (let ((ptr (jit-state-ptr self)))
    ;;(format t "initialize-instance self:~A ptr:~A~%" self ptr)
    (tg:finalize self (lambda () (free-state-ptr ptr)))))
...
;;(defcfun "fn_jit_clear_state" :void (jit jit-state-t-ptr))
;;(defcfun "fn_jit_destroy_state" :void (jit jit-state-t-ptr))
;; the functions #'FN-JIT-CLEAR-STATE-RAW and #'FN-JIT-DESTROY-STATE-RAW have to operate on the raw pointer (of type "jit_state_t*"), because the function passed to #'TG:FINALIZE must not close over the JIT-STATE object.
(defcfun ("fn_jit_clear_state" fn-jit-clear-state-raw) :void (jit :pointer))
(defcfun ("fn_jit_destroy_state" fn-jit-destroy-state-raw) :void (jit :pointer))
...
(defcfun "fn_jit_get_code" jit-pointer-t (jit jit-state) (code-size-ptr jit-word-t-ptr))
(defcfun "fn_jit_set_code" :void (jit jit-state) (code jit-pointer-t) (size jit-word-t))
(defcfun "fn_jit_get_data" jit-pointer-t (jit jit-state) (data-size-ptr jit-word-t-ptr) (note-size-ptr jit-word-t-ptr))
...
(defcfun "fn_jit_print" :void (jit jit-state))
...
(defmacro def-new-node-w (basename)
  `(def-new-node ,basename (jit jit-state) (u jit-word-t)))
(def-new-node-w live)
...
(defmacro def-new-node-ww (basename)
  `(def-new-node ,basename (jit jit-state) (u jit-word-t) (v jit-word-t)))
...
(def-new-node-www ldxi_c)
(def-new-node-www ldxr_uc)
(def-new-node-www ldxi_uc)
(def-new-node-www ldxr_s)
(def-new-node-www ldxi_s)
(def-new-node-www ldxr_us)
(def-new-node-www ldxi_us)
(def-new-node-www ldxr_i)
(def-new-node-www ldxi_i)
(def-new-node-www stxr_c)
(def-new-node-www stxi_c)
(def-new-node-www stxr_s)
(def-new-node-www stxi_s)
(def-new-node-www stxr_i)
(def-new-node-www stxi_i)
...
(def-new-node-www ldxi_f)
(def-new-node-www stxr_f)
(def-new-node-www stxi_f)
...
(if-wordsize-32
  ...
  (def-new-node-www ldxi))
(if-wordsize-64
  (def-new-node-www ldxr_ui)
  (def-new-node-www ldxi_ui)
  (def-new-node-www ldxr_l)
  (def-new-node-www ldxi_l)
  (def-new-node-www stxr_l)
  (def-new-node-www stxi_l)
  (def-new-node-www ldxr)
  (def-new-node-www ldxi)
  (def-new-node-www stxr)
  (def-new-node-www stxi))
...
(defcfun "fn_jit_disassemble" :void (jit jit-state))
